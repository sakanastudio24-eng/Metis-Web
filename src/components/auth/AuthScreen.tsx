...
import { ArrowRight, Github, LoaderCircle, Mail } from "lucide-react";
    ...
    startTransition(async () => {
      if (provider === "google" && isTemporaryGoogleEnabled) {
        try {
          ...
          setNotice(sharedCopy.temporaryAccessError, "error");
          return;
        }
      }
      ...
      <div className="space-y-5">
        <div className="space-y-3">
          ...
          <div className="rounded-[24px] border border-white/10 bg-white/5 p-4">
            <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-white/35">{sharedCopy.brandLabel}</p>
            <p className="mt-2 text-sm leading-6 text-white/70">{sharedCopy.stageBody}</p>
          </div>
        </div>
        ...
        {isTemporaryGoogleEnabled ? (
          <div className="rounded-[24px] border border-[#dc5e5e]/20 bg-[#dc5e5e]/10 p-4">
            <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-[#ffb8b8]">
              {sharedCopy.temporaryAccessLabel}
            </p>
            <p className="mt-2 text-sm leading-6 text-white/76">{sharedCopy.temporaryAccessBody}</p>
            <p className="mt-3 text-sm font-medium text-[#ffb8b8]">{sharedCopy.temporaryAccessAction}</p>
          </div>
        ) : null}
