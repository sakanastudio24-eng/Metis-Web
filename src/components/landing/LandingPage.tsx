...
export function LandingPage() {
  ...
  return (
    <div style={{ minHeight: "100vh", position: "relative", background: CREAM }}>
      <LandingChromeClient />
      ...
      >
        <div
          ...
          style={{ maxWidth: 1340, margin: "0 auto", padding: "0 24px", display: "flex", gap: 60, alignItems: "flex-start" }}
        >
          <div style={{ flex: 1, minWidth: 0 }}>
            <section id="product" style={{ padding: "100px 0" }}>
              ...
              <div style={{ paddingTop: 80 }}>
                ...
                <h2 style={{ fontFamily: FONT_SERIF, fontSize: "clamp(36px, 4.5vw, 56px)", color: TEXT_W, lineHeight: 1.1, letterSpacing: "-0.025em", margin: 0, marginBottom: 16, maxWidth: 560 }}>
                  {copy.product.heading}
                </h2>
                <p style={{ fontFamily: FONT_SANS, fontSize: 17, color: TEXT_W_DIM, lineHeight: 1.6, margin: 0, marginBottom: 56, maxWidth: 480 }}>
                  {copy.product.body}
                </p>
                <div className="metis-grid-2" style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: 16 }}>
                  {copy.product.features.map(({ title, desc }) => (
                    <div key={title} style={{ background: CARD_BG, border: `1px solid ${CARD_BD}`, borderRadius: 16, padding: 24 }}>
            ...
            <section id="problem" style={{ padding: "100px 0" }}>
              ...
              <div style={{ paddingTop: 80 }}>
                ...
                <h2 style={{ fontFamily: FONT_SERIF, fontSize: "clamp(36px, 4.5vw, 56px)", color: TEXT_W, lineHeight: 1.1, letterSpacing: "-0.025em", margin: 0, marginBottom: 16, maxWidth: 580 }}>
                  {copy.problem.heading}
                </h2>
                <p style={{ fontFamily: FONT_SANS, fontSize: 17, color: TEXT_W_DIM, lineHeight: 1.65, margin: 0, marginBottom: 56, maxWidth: 520 }}>
                  {copy.problem.body}
                </p>
                <div className="metis-grid-2-tight" style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: 12, marginBottom: 48 }}>
                  {copy.problem.stats.map(({ value, unit, label }) => (
                    <div key={label} style={{ background: "rgba(0,0,0,0.15)", border: "1px solid rgba(255,255,255,0.15)", borderRadius: 14, padding: "20px 22px" }}>
                      <div style={{ display: "flex", alignItems: "baseline", gap: 4, marginBottom: 6 }}>
                        <span style={{ fontFamily: FONT_DISPLAY, fontSize: 32, color: TEXT_W, lineHeight: 1 }}>{value}</span>
                        {unit ? <span style={{ fontFamily: FONT_SANS, fontSize: 14, color: TEXT_W_DIM }}>{unit}</span> : null}
                      </div>
            ...
            <section id="fixes" style={{ padding: "100px 0" }}>
              ...
              <div style={{ paddingTop: 80 }}>
                ...
                <h2 style={{ fontFamily: FONT_SERIF, fontSize: "clamp(36px, 4.5vw, 56px)", color: TEXT_W, lineHeight: 1.1, letterSpacing: "-0.025em", margin: 0, marginBottom: 16, maxWidth: 540 }}>
                  {copy.fixes.heading}
                </h2>
                <p style={{ fontFamily: FONT_SANS, fontSize: 17, color: TEXT_W_DIM, lineHeight: 1.65, margin: 0, marginBottom: 52, maxWidth: 480 }}>
                  {copy.fixes.body}
                </p>
                <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                  {copy.fixes.items.map(({ rank, title, color, saving, rootCause, fix }) => (
                    <div key={title} style={{ borderRadius: 16, overflow: "hidden", border: `1px solid ${rank === 1 ? color + "55" : color + "25"}` }}>
            ...
            <section id="solution" style={{ padding: "100px 0 80px" }}>
              ...
              <div style={{ paddingTop: 80 }}>
                ...
                <h2 style={{ fontFamily: FONT_SERIF, fontSize: "clamp(36px, 5vw, 64px)", color: TEXT_W, lineHeight: 1.08, letterSpacing: "-0.03em", margin: 0, marginBottom: 20, maxWidth: 560 }}>
                  {copy.solution.heading[0]}
                  <br />
                  {copy.solution.heading[1]}
                </h2>
                <p style={{ fontFamily: FONT_SANS, fontSize: 17, color: TEXT_W_DIM, lineHeight: 1.65, margin: 0, marginBottom: 44, maxWidth: 460 }}>
                  {copy.solution.body}
                </p>
                  ...
                  {copy.solution.checklist.map((item) => (
                    <div key={item} style={{ display: "flex", alignItems: "center", gap: 12 }}>
                      <div style={{ width: 22, height: 22, borderRadius: "50%", background: "rgba(34,197,94,0.15)", border: "1px solid rgba(34,197,94,0.3)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                        <span style={{ color: "#4ade80", fontFamily: FONT_SANS, fontSize: 12 }}>✓</span>
                      </div>
                ...
                <div style={{ display: "inline-flex", alignItems: "center", gap: 20, borderRadius: 18, padding: "18px 24px", background: "rgba(34,197,94,0.1)", border: "1px solid rgba(34,197,94,0.25)", marginBottom: 40 }}>
                  <svg width={60} height={60} viewBox="0 0 60 60" style={{ flexShrink: 0 }}>
                    ...
                    <circle cx={30} cy={30} r={23} fill="none" stroke="#22c55e" strokeWidth={5} strokeDasharray={2 * Math.PI * 23} strokeDashoffset={2 * Math.PI * 23 * (1 - landingAnalysis.resultScore / 100)} strokeLinecap="round" transform="rotate(-90 30 30)" />
                    <text x={30} y={30} textAnchor="middle" dominantBaseline="central" fill="white" fontSize={14} fontFamily={FONT_DISPLAY}>{landingAnalysis.resultScore}</text>
                  </svg>
                  <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                      <span style={{ fontFamily: FONT_DISPLAY, fontSize: 28, color: TEXT_W, lineHeight: 1 }}>{landingAnalysis.resultScore}</span>
                      <span style={{ borderRadius: 999, padding: "4px 12px", background: "rgba(34,197,94,0.2)", color: "#22c55e", fontFamily: FONT_SANS, fontSize: 12, fontWeight: 600 }}>{copy.solution.resultCaption}</span>
                    </div>
      ...
      >
        <div style={{ position: "absolute", inset: 0, pointerEvents: "none", backgroundImage: "radial-gradient(circle at 20% 50%, rgba(220,94,94,0.06) 0%, transparent 60%), radial-gradient(circle at 80% 20%, rgba(99,102,241,0.05) 0%, transparent 50%)" }} />
          ...
          <div className="metis-footer-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 80, alignItems: "center" }}>
            <div>
              <div style={{ display: "inline-flex", alignItems: "center", gap: 6, borderRadius: 999, padding: "4px 12px", background: "rgba(220,94,94,0.15)", border: "1px solid rgba(220,94,94,0.25)", marginBottom: 24 }}>
                ...
                <span style={{ fontFamily: FONT_SANS, fontSize: 11, fontWeight: 600, color: RED, textTransform: "uppercase", letterSpacing: "0.08em" }}>{copy.footer.badge}</span>
              </div>
