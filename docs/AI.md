...
Temporary auth review paths are gone. All live auth now runs through the real passwordless flow.
...
Account deletion is a website-owned soft-delete flow. It must stay deliberate, require fresh magic-link re-auth, and never become a one-click action.
